...
dynamic  Använder serverns botcontrol-kommando för on/off/status/min/max/extra bot.
```
...
add_ct lägger till en extra bot i CT.
add_t lägger till en extra bot i T.
del_ct tar bort extra bot från CT.
del_t tar bort extra bot från T.
...
```json
{
  "type": "BOTCONTROL",
  "accepted": true,
  "message": "Bot Control is ON. Min: 4. Max: 10. Extra: CT"
}
```

#### BOTCONTROL dynamic extra bot

Lägger till eller tar bort en extra bot i CT eller T. Detta kan användas som handikapp när ett lag är dominerande.

Request för extra bot i CT:
...
Request för att ta bort extra bot från CT:
...
Request för att ta bort extra bot från T:
...
curl -X POST \
  -H 'Authorization: Bearer dev-secret-change-me' \
  -H 'Content-Type: application/json' \
  -d '{"type":"BOTCONTROL","arguments":{"type":"dynamic","command":"add_ct"}}' \
  http://localhost:8080/api/v1/server/commands
```
